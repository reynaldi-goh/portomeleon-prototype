...
from agent import build_agent, select_action, train_step, update_target_network, ReplayBuffer
...
def run_stepper(env, q_net, n_actions):
    ...
    while not done:
        ...
        action = select_action(obs, q_net, epsilon=0.0, n_actions=n_actions)
        obs, reward, terminated, truncated, info = env.step(action)
        done = terminated or truncated

        wanted = ACTION_NAMES[action]
        did = ACTION_NAMES[info["effective_action"]]
        print(f"Bot wanted: {wanted} | Actually did: {did}")
